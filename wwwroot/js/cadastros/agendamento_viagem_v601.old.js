...
// Corrected Event Handler for btnConfirma
$("#btnConfirma").off("click").on("click", async function (event) {
    ...
    const recorrenciaViagemId = document.getElementById("txtRecorrenciaViagemId").value || null;

    try {
        if (recorrenciaViagemId === null || recorrenciaViagemId === "" || recorrenciaViagemId === "00000000-0000-0000-0000-000000000000") {
            // First entry of the series: Ask about editing all
            await editarAgendamentoRecorrente(viagemId, false);
        } else {
            // Intermediate or subsequent entry: Ask about editing all subsequent
            await editarAgendamentoRecorrente(viagemId, true);
        }
    } catch (error) {
...
// Define this function as async
async function enviarAgendamentoComOpcao(viagemId, editarTodos, editarProximos) {
    try {
        const agendamento = criarAgendamento(); // Create the agendamento object

        // Set properties based on user choice
...
async function executarEdicao() {
    ...
    function criarAgendamento(dataInicial, viagemIdRecorrente = null) {
        let viagemId = document.getElementById("txtViagemId").value;
        ...
        // Definindo quais dias da semana foram selecionados
        const diasSemana = {
...
async function obterEDefinirDatasCalendario(viagem) {
    try {
        console.log("Testando chamada a getDatasIniciais...");
        const datasIniciais = await getDatasIniciais(viagem);
...
// Envia o objeto de agendamento ao backend
...
function ajustarDataInicialRecorrente(tipoRecorrencia) {
    ...
    // Correção do mapeamento para que fique compatível com o momento.js
...
function ExibeViagem(viagem) {
    ...
    if (viagem === "") {
        ...
        if (viagem.intervalo === 'V') {
            ...
            obterEDefinirDatasCalendario(viagem.viagemId);
        }
