...
// Supondo que você usa Syncfusion ComboBox no #lstPeriodos
function rebuildLstPeriodos()
{
    new ej.dropdowns.DropDownList({
        dataSource: dataPeriodos,
        fields: { value: "PeriodoId", text: "Periodo" }, // ajuste conforme seu objeto!
            ...
            if (!viagemId)
            {
                if (periodoRecorrente === null)
                {
                    try
                    {
                        ...
                        {
                            toastr.error("Erro ao Criar Agendamento");
        ...
        return new Promise((resolve, reject) =>
        {
            try
            {
                $.ajax({
                    ...
                    error: function (err)
                    {
                        TratamentoErroComLinha("agendamento_viagem<num>.js", "(error)", err);
                    }
                });
            }
            catch (error)
...
async function enviarAgendamentoComOpcao(viagemId, editarTodos, editarProximos, dataInicial = null, viagemIdRecorrente = null)
{
    try
    {
        try
        {
            ...
            if (objViagem)
            {
                toastr.success('Agendamento atualizado com sucesso', { timeOut: 3000 });
            }
        }
        catch (error)
        {
    ...
    try
    {
        ...
        if (calendarObj)
        {
            ...
            calendarObj.renderDayCell = function (args)
            {
                ...
                {
                    TratamentoErroComLinha("agendamento_viagem<num>.js", "(calendarObj.renderDayCell)", error);
                }
            };
            ...
            calendarObj.change = function (args)
            {
                ...
                {
                    TratamentoErroComLinha("agendamento_viagem<num>.js", "(calendarObj.change)", error);
                }
            };
...
function exibirErroAgendamento()
{
    try
    {
        toastr.error('Não foi possível criar o agendamento com os dados informados', { timeOut: 3000 });
...
function exibirMensagemSucesso()
{
    try
    {
        toastr.success('Todos os agendamentos foram criados com sucesso', { timeOut: 3000 });
        ...
        $.ajax({
            ...
            success: function (response)
            {
                try
                {
                    ...
                    // --- Aqui garante que o container fique visível e com altura correta ---
...
$("#btnViagem").click(function (event)
{
    try
    {
        ...
        //    error: function (err)
        //    {
        //        Alerta.Erro("Erro ao buscar número da última ficha", "Classe: agendamento_viagem<num>.js | Método: document.ready | Erro: " + err.statusText);
        //    }
        //});
    } catch (error)
    {
...
$("#txtDataInicial").focusout(function ()
{
    try
    {
        ...
        if (!valDataInicial || !moment(valDataInicial).isValid() || valDataInicial === null)
        {
            lstDataInicial.value = moment().toDate(); // ou new Date()
            lstDataInicial.dataBind(); // força atualizar o componente
            return;
        }
...
{
    if (document.getElementById('lstFinalidade').ej2_instances[0] === null) return;
...
document.addEventListener("DOMContentLoaded", function ()
{
    try
    {
        ...
        var dropDownListObj = new ej.dropdowns.DropDownList({
            ...
            fields: { value: "finalidadeId", text: "descricao" },
            placeholder: "Selecione a finalidade",
            allowFiltering: true,
            showClearButton: true,
            sortOrder: "Ascending"
        });
        dropDownListObj.appendTo('#lstFinalidade');
    } catch (error)
    {
        TratamentoErroComLinha("agendamento_viagem<num>.js", "(anonymous)", error);
    }
...
async function validarQuilometragem()
{
    try
    {
        ...
        if (txtKmInicial === "")
        {
            Alerta.Erro("Atenção", "A quilometragem inicial é obrigatória");
        ...
        if (txtKmFinal !== "")
        {
            ...
            if (!isNaN(kmInicial) && !isNaN(kmFinal))
            {
                ...
                if (diferencaKm >= 100)
                {
                    const confirmacao = await Alerta.Confirmar(
                        "Atenção",
                        'A quilometragem final está 100 km ou mais acima da inicial. Tem certeza?',
                        "Tenho certeza! 💪🏼",
                        "Me enganei! 😟"
                    );
    ...
    try
    {
        ...
        $.ajax({
            ...
            success: function (res)
            {
                ...
                {
                    Alerta.Alerta("Alerta na Ficha de Vistoria", "O número inserido difere em ±100 da última Ficha inserida");
                }
            }
        });
        ...
        $.ajax({
            ...
            success: function (res)
            {
                if (res.data === true)
                {
                    Alerta.Alerta("Alerta na Ficha de Vistoria", "Já existe uma Ficha inserida com esta numeração");
                }
            }
        });
    } catch (error)
...
function limparCamposModalViagens()
{
    try
    {
        ...
        // --------- Limpa NumericTextBox Syncfusion ---------
        if (window.txtDuracao && txtDuracao.ej2_instances) txtDuracao.ej2_instances[0].value = null;
        if (window.txtQuilometragem && txtQuilometragem.ej2_instances) txtQuilometragem.ej2_instances[0].value = null;

        // --------- Limpa ComboBoxes/DropDownTrees ---------
        const syncIds = [
            "cmbOrigem", "cmbDestino", "lstMotorista", "lstVeiculo", "lstRequisitante", "ddtSetor",
            "lstEventos", "ddtCombustivelInicial", "ddtCombustivelFinal", "lstDiasMes"
        ];
        syncIds.forEach(id =>
        {
            let el = document.getElementById(id);
            if (el && el.ej2_instances?.[0])
            {
                el.ej2_instances[0].value = null;
                if (el.ej2_instances[0].enabled !== undefined) el.ej2_instances[0].enabled = true;
                if (typeof el.ej2_instances[0].dataBind === "function") el.ej2_instances[0].dataBind();
            }
        });

        // --------- Reset MultiSelect Syncfusion: #lstDias ---------
        let lstDias = document.getElementById("lstDias");
        if (lstDias && lstDias.ej2_instances?.[0])
        {
            lstDias.ej2_instances[0].value = [];
            lstDias.ej2_instances[0].enabled = true;
            if (typeof lstDias.ej2_instances[0].dataBind === "function") lstDias.ej2_instances[0].dataBind();
        }

        // --------- Reset para campo criado via JS: #lstFinalidade ---------
        let lstFinalidade = document.getElementById("lstFinalidade");
        if (lstFinalidade && lstFinalidade.ej2_instances?.[0])
        {
            lstFinalidade.ej2_instances[0].value = null;
            lstFinalidade.ej2_instances[0].enabled = true;
            if (typeof lstFinalidade.ej2_instances[0].dataBind === "function") lstFinalidade.ej2_instances[0].dataBind();
        }

        // --------- Reset para ComboBox Recorrente ---------
        let lstRecorrente = document.getElementById("lstRecorrente");
        if (lstRecorrente && lstRecorrente.ej2_instances?.[0])
        {
            lstRecorrente.ej2_instances[0].value = 'N'; // Define como NÃO recorrente (ajuste conforme padrão)
            lstRecorrente.ej2_instances[0].enabled = true;
            if (typeof lstRecorrente.ej2_instances[0].dataBind === "function") lstRecorrente.ej2_instances[0].dataBind();
        }

        // --------- Reset/reconstrução do ComboBox Periodos ---------
        let elPeriodos = document.getElementById("lstPeriodos");
        if (elPeriodos && elPeriodos.ej2_instances?.[0])
        {
            elPeriodos.ej2_instances[0].value = null;
            elPeriodos.ej2_instances[0].enabled = true;
            if (typeof elPeriodos.ej2_instances[0].dataBind === "function") elPeriodos.ej2_instances[0].dataBind();
        } else
        {
            rebuildLstPeriodos && rebuildLstPeriodos(); // Certifique-se que esta função existe!
        }

        // --------- RichTextEditor Syncfusion ---------
        if (window.rteDescricao && rteDescricao.ej2_instances)
        {
            rteDescricao.ej2_instances[0].value = '';
        }

        // --------- Outros campos de eventos, accordion ---------
        if (window.lstRequisitanteEvento && lstRequisitanteEvento.ej2_instances) lstRequisitanteEvento.ej2_instances[0].value = null;
        if (window.lstSetorRequisitanteEvento && lstSetorRequisitanteEvento.ej2_instances) lstSetorRequisitanteEvento.ej2_instances[0].value = null;
        if (window.ddtSetorRequisitante && ddtSetorRequisitante.ej2_instances) ddtSetorRequisitante.ej2_instances[0].value = null;

        // --------- Containers, DIVs e extras ---------
        ...
        // --------- Limpa listas/calendários customizados ---------
        let cal = document.getElementById("calDatasSelecionadas");
        if (cal && cal.ej2_instances?.[0]) cal.ej2_instances[0].values = [];
        let lstDiasHTML = document.getElementById("lstDiasCalendarioHTML");
        if (lstDiasHTML) lstDiasHTML.innerHTML = "";

        // --------- Limpa e esconde o relatório ---------
        $("#ReportContainer").hide();
        $("#fichaReport").html(''); // Limpa qualquer conteúdo anterior, inclusive "Carregando..."

        // Opcional: remove qualquer classe de loading/spinner se usar
        $("#fichaReport").removeClass('loading');

        // --------- LIMPA INDICADORES DE CARREGAMENTO ---------
        // Garante que não fique "Carregando..." do último relatório
        $("#ReportContainer").hide().removeClass('visible').css('height', '0');
        $("#fichaReport").html('');

        // --------- Se necessário: aborte requisições AJAX de relatório ---------
        // Se você usa alguma variável global tipo xhrRelatorio, aborte:
        if (window.xhrRelatorio && window.xhrRelatorio.abort) window.xhrRelatorio.abort();

        // --------- Se precisar: reseta estados extras ou triggers customizados aqui ---------
    }
    catch (error)
    {
        TratamentoErroComLinha("agendamento_viagem<num>.js", "limparCamposModalViagens", error);
    }
}