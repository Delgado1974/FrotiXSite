    ...
    try {
        if (!viagemId) {
            ...
            if (periodoRecorrente === 'V') {
                // Mantendo a lógica existente para "Dias Variados"
                ...
                let primeiroAgendamento = criarAgendamento(null, null);
                ...
                if (selectedDates.length > 1) {
                    for (let i = 1; i < selectedDates.length; i++) {
                        try {
                            const agendamentoSubsequente = criarAgendamento(viagemId, recorrenciaViagemId, moment(selectedDates[i]).toISOString().split('T')[0]);
                            await delay(500);
            ...
            } else if (['S', 'Q', 'M'].includes(periodoRecorrente)) {
                // Refatorando a lógica para "Semanal", "Quinzenal" e "Mensal"
                ...
                // Mantendo a lógica existente para "Diário"
                const datasRecorrentes = ajustarDataInicialRecorrente(periodoRecorrente);
                await handleRecurrence(periodoRecorrente, datasRecorrentes);
                exibirMensagemSucesso();
            } else {
                // Adicionar agendamento único;
                const agendamento = criarAgendamento(null,null,null);
                await enviarNovoAgendamento(agendamento);
            }
        } else {
            // Editar agendamento existente

            let objViagem = recuperarViagemEdicao(viagemId);
            ...
            if (objViagem.Recorrente === "S") {
                ...
                if (result.isConfirmed) {
                    await editarAgendamento(viagemId, false);
                } else {
                    await editarAgendamento(viagemId, true);
                }
            } else
            {
                const agendamentoUnicoAlterado = await recuperarViagemEdicao(viagemId);
                await editarAgendamento(agendamentoUnicoAlterado);
            }
...
}); // Correct closure for .on("click") event
...
async function obterAgendamentosRecorrentes(viagemId, DataInicialRecorrencia) {
    try {
        const response = await fetch(`/api/agenda/${viagemId}`);
        if (!response.ok) {
            throw new Error('Erro ao obter agendamentos recorrentes');
        }
        return await response.json();
    } catch (error) {
        console.error('Erro ao obter agendamentos recorrentes:', error);
        throw error;
    }
}

async function editarAgendamento(viagemId, editaTodos) {
    try {
        if (!viagemId) {
        ...
        if (editaTodos) {
            ...
            const agendamentosRecorrentes = await obterAgendamentosRecorrentes(viagemId);
            ...
            var dataToPost = JSON.stringify(objViagem);
            var url = '/api/Agenda/Agendamento';
            $.ajax({
                url: url,
                type: "POST",
                data: dataToPost,
            ...
            //    const agendamentoAtual = await obterAgendamento(viagemId);
        ...
        Swal.fire({
            title: 'Erro',
            text: 'Erro ao editar o agendamento.',
            icon: 'error',
            confirmButtonText: 'Ok',
        });
    }
...
async function atualizarAgendamento(agendamento) {
    try {
        ////    const response = await fetch(`/api/agenda/ObterAgendamento/${viagemId}`, {
        ////        method: 'PUT',
        ////        headers: {
...
async function recuperarViagemEdicao(viagemId) {
    try {
        const response = await $.ajax({
            ...
            type: "GET",
            data: { viagemId: viagemId }, // Isso se tornará parte da query string
            dataType: "json"
        });
