...
$("#btnConfirma").off("click").on("click", async function (event) {
    ...
    $(this).prop("disabled", true); // Desabilitar o botão para evitar múltiplas chamadas
    ...
    try {
        if (!viagemId) {
            ...
            if (periodoRecorrente === 'V') {
                ...
                // Enviar o primeiro agendamento e salvar o viagemId e recorrenciaViagemId gerados
                let primeiroAgendamento = criarAgendamento(dataInicial,null);
                primeiroAgendamento.DataInicial = moment(selectedDates[0]).toISOString().split('T')[0];
                let agendamentoObj;

                try {
                    agendamentoObj = await enviarNovoAgendamento(primeiroAgendamento, selectedDates.length === 1);
                    if (!agendamentoObj || !agendamentoObj['viagemId']) {
                        throw new Error("Erro ao criar o primeiro agendamento. ViagemId não gerado.");
                    }
                } catch (error) {
                ...
                // Gerar um identificador de recorrência para o conjunto de agendamentos
                let recorrenciaViagemId = agendamentoObj['viagemId'];

                // Criar agendamentos subsequentes para as demais datas (se houver mais de uma data)
                if (selectedDates.length > 1) {
                    for (let i = 1; i < selectedDates.length; i++) {
                        try {
                            const agendamentoSubsequente = criarAgendamento(moment(selectedDates[i]).toISOString().split('T')[0], recorrenciaViagemId);
                            //agendamentoSubsequente.DataInicial = moment(selectedDates[i]).toISOString().split('T')[0];
                            //agendamentoSubsequente.RecorrenciaViagemId = recorrenciaViagemId;

                            // Adicionando um atraso de 500ms entre as chamadas
                            await delay(500);

                            // Enviar agendamento subsequente
                            let response = await enviarNovoAgendamento(agendamentoSubsequente, i === selectedDates.length - 1);
                            if (!response || !response.operacaoBemSucedida) {
                                console.error(`Erro ao criar o agendamento subsequente para a data ${selectedDates[i]}`);
                                throw new Error("Erro ao criar agendamento subsequente.");
                            }
                        } catch (error) {
                            console.error("Erro ao processar agendamento subsequente:", error);
                            Swal.fire({
                                title: 'Erro',
                                text: `Erro ao processar o agendamento subsequente para a data ${selectedDates[i]}.`,
                                icon: 'error',
                                confirmButtonText: 'Ok'
                            });
                            $(this).prop("disabled", false);
                ...
                // Para outros períodos recorrentes
                const datasRecorrentes = ajustarDataInicialRecorrente(periodoRecorrente);
                if (!datasRecorrentes || datasRecorrentes.length === 0) {
                    console.error("Nenhuma data inicial válida retornada para outros períodos.");
                    $(this).prop("disabled", false);
                    return;
                }

                // Enviar o primeiro agendamento e salvar o viagemId gerado
                let primeiroAgendamento = criarAgendamento(datasRecorrentes[0]);
                let agendamentoObj = await enviarNovoAgendamento(primeiroAgendamento, datasRecorrentes.length === 1);

                // Verificar se o agendamentoObj foi retornado corretamente
                if (!agendamentoObj || !agendamentoObj.ViagemId) {
                ...
                if (datasRecorrentes.length > 1) {
                    for (let i = 1; i < datasRecorrentes.length; i++) {
                        const agendamentoSubsequente = criarAgendamento(datasRecorrentes[i]);
                        agendamentoSubsequente.RecorrenciaViagemId = agendamentoObj.ViagemId;

                        await enviarNovoAgendamento(agendamentoSubsequente, i === datasRecorrentes.length - 1);
                    }
                }
            } else {
                // Adicionar agendamento único
                const agendamento = criarAgendamento();
                await enviarNovoAgendamento(agendamento);
            }
        } else {
            // Editar agendamento existente
            const isSubsequente = recorrenciaViagemId !== null && recorrenciaViagemId !== "" && recorrenciaViagemId !== "00000000-0000-0000-0000-000000000000";
            const { isConfirmed } = await Swal.fire({
                title: 'Editar agendamento recorrente',
                text: isSubsequente ? "Você deseja editar apenas este agendamento ou todos os próximos agendamentos desta série?" : "Você deseja editar apenas este agendamento ou todos os agendamentos desta série?",
                icon: 'question',
            ...
            await enviarAgendamentoComOpcao(viagemId, !isConfirmed, isSubsequente);
        }
    } catch (error) {
...
// Definir a função de atraso (delay)
//const delay = ms => new Promise(resolve => setTimeout(resolve, ms));


// Função específica para enviar agendamento ao v551
//async function enviarAgendamentoSerialV551(agendamento) {
//    if (isSubmitting) return;
//    isSubmitting = true;
//    $("#btnConfirma").prop("disabled", true);

//    try {
//        // Enviar um único agendamento para o v551
//        const response = await enviarAgendamentoV551(agendamento);
//        if (response) {
//            console.log("Agendamento criado com sucesso no v551.");
//        }
//        return response;
//    } catch (error) {
//        console.error("Erro ao enviar agendamento para o v551:", error);
...
//async function enviarAgendamentoV551(agendamento) {
//    return await $.ajax({
//        type: "POST",
...
//    return await $.ajax({
//        type: "POST",
//        url: "/api/Agenda/AgendamentoV551",
//        contentType: "application/json; charset=utf-8",
//        dataType: "json",
//        data: JSON.stringify(agendamento)
//    });
...
async function enviarAgendamentoComOpcao(viagemId, editarTodos, editarProximos) {
    try {
        const agendamento = criarAgendamento();
...
// Função para buscar datas e atualizar o ListBox
function buscarEAtualizarListBox(viagem) {
    const viagemId = viagem.viagemId;
    const recorrenciaViagemId = viagem.recorrenciaViagemId || null;

    // Faz a chamada AJAX para obter as datas de agendamento
    $.ajax({
        url: 'api/Agenda/GetDatasViagem',
        type: "GET",
        data: {
            viagemId: viagemId,
            recorrenciaViagemId: recorrenciaViagemId // Passa recorrenciaViagemId se estiver definido
        },
        ...
        success: function (datas) {
            if (datas && Array.isArray(datas)) {
                ...
                console.error("Erro ao obter datas:", datas);
            }
        },
...
//// Definir uma função assíncrona para verificar a edição de um agendamento recorrente existente
//async function verificarEdicaoRecorrente(viagemId, periodoRecorrente) {
//    let editarTodosRecorrentes = false;

//    // Verificar se é edição de um agendamento recorrente existente
...
async function executarEdicao() {
    ...
    const editarTodosRecorrentes = await verificarEdicaoRecorrente(viagemId, periodoRecorrente);
...
//    try {
//        const agendamento = criarAgendamento(document.getElementById("txtDataInicial").ej2_instances[0].value, viagemId);
//        agendamento.editarTodosRecorrentes = editarTodosRecorrentes;  // Adicionar novo parâmetro

//        await enviarAgendamento(agendamento, editarTodosRecorrentes); // Passar como parâmetro
//        exibirMensagemSucesso();
//    } catch (error) {
//        console.error("Erro ao criar/agendar:", error);
//        Swal.fire({
//            title: 'Erro',
//            text: 'Erro ao criar ou editar agendamento.',
//            icon: 'error',
//            confirmButtonText: 'Ok'
//        });
//    } finally {
...
async function getDatasIniciais(viagemId, recorrenciaViagemId) {
    ...
    //if (!viagemId_AJAX || !recorrenciaViagemId_AJAX) {
    //    console.error("Erro: ViagemId ou RecorrenciaViagemId não são válidos.");
    //    throw new Error("ViagemId ou RecorrenciaViagemId inválido");
    //}
...
// Function to update the calendar and ListBox with selected dates
...
// Função para enviar agendamento sem necessidade de retorno
async function enviarAgendamento(agendamento) {
    try {
        ...
        // Se chegou aqui, a requisição foi realizada com sucesso
        console.log("Agendamento enviado com sucesso.");

        // Acessando a propriedade novaViagem do objeto data
        const novaViagem = agendamento.novaViagem;

        // Imprimindo os detalhes da novaViagem
        console.log("ID da Viagem:", novaViagem.viagemId);
        console.log("Data Inicial:", novaViagem.dataInicial);
        console.log("Hora Início:", novaViagem.horaInicio);
        console.log("Status:", novaViagem.status);

        return novaViagem; // Aqui você retorna o resultado da função principal
    } catch (error) {
        console.error("Erro ao enviar agendamento:", error);
    }
}
...
function ajustarDataInicialRecorrente(tipoRecorrencia) {
    ...
    let dataAtual = moment(document.getElementById("txtDataInicial").ej2_instances[0].value).toISOString().split('T')[0]
    const dataFinal = moment(document.getElementById("txtFinalRecorrencia").ej2_instances[0].value).toISOString().split('T')[0]
...
function criarAgendamento(viagemIdRecorrente = null) {
    // Validação rigorosa dos parâmetros antes de criar o agendamento
    if (!dataInicial) {
        console.error("Data inicial inválida para agendamento");
        throw new Error("Data inicial inválida para agendamento");
    }

    if (viagemIdRecorrente === undefined) {
        console.error("RecorrenciaViagemId não pode ser undefined");
        throw new Error("RecorrenciaViagemId inválido");
    }

    ///* Inicializando valores padrão para viagemId*/
    //let viagemId = viagemIdRecorrente || '00000000-0000-0000-0000-000000000000';
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateBadge() {
        ...
        badgeHTML.textContent = selectedDates.length;
    }
