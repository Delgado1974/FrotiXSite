...
$("#btnConfirma").off("click").on("click", async function (event) {
    ...
    try {
        ...
        if (viagemId) {
            ...
            editarTodos = !result.isConfirmed;
            // Dados a serem enviados para o backend no modo edição
            agendamento = {
                viagemId: viagemId,
                recorrente: periodoRecorrente === 'S' ? "S" : "N",
                datasSelecionadas: periodoRecorrente === 'M' ? ajustarDataInicialRecorrente(periodoRecorrente, edicao) : [],
                editarTodosRecorrentes: editarTodos
            };
        } else {
            // Dados a serem enviados para o backend no modo adição
...
async function editarAgendamentoRecorrente(viagemId, editaTodos, dataInicialRecorrencia, recorrenciaViagemId) {
    if (!viagemId) {
...
function ajustarDataInicialRecorrente(tipoRecorrencia, edicao) {
    const datas = [];
    ...
    if (edicao)
    {
        dataFinal = document.getElementById('txtDataFinalRecorrencia').value;
    }
    else
    {
        dataFinal = document.getElementById("txtFinalRecorrencia")?.ej2_instances?.[0]?.value;
    }
...
async function recuperarViagemEdicao(recorrenciaViagemId) {
    try {
        const response = await $.ajax({
            url: '/api/Agenda/ObterAgendamentoEdicao',
            type: "GET",
            data: { recorrenciaViagemId: recorrenciaViagemId }, // Isso se tornará parte da query string
            //dataType: "json"
        });

        const objViagem = response[0]
        return objViagem;
    } catch (err) {
        console.error(err);
        alert('Something went wrong');
