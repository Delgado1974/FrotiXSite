...
$('#modalAjustaCustos').on('shown.bs.modal', function (event) {
    ...
    var RowNumber = $(event.relatedTarget).closest("tr").find("td:eq(15)").text() - 1;
    ...
    //Pega Motorista e Veículo da Viagem
    $.ajax({
        ...
        success: function (res) {
            if (res.success === true) {
                ...
                document.getElementById("lstSetorSolicitanteAlterado").ej2_instances[0].value = [res.setorsolicitanteId];

                var evento = document.getElementById('lstEvento').ej2_instances[0];
                document.getElementById("lstEvento").ej2_instances[0].value = [res.eventoId];
    ...
    //    var ddtEventos = document.getElementById("lstEvento").ej2_instances[0];
    //    ddtEventos.enabled = true; // To disables
    //    ddtEventos.value = evento.eventoId[0];
...
$("#btnAjustarViagem").click(function (e) {
    ...
    if (evento.value != null) {
        var objViagem = JSON.stringify({ "ViagemId": $('#txtId').val(), "NoFichaVistoria": $('#txtNoFichaVistoria').val(), "DataInicial": $('#txtDataInicial').val(), "HoraInicial": $('#txtHoraInicial').val(), "KmInicial": $('#txtKmInicial').val(), "DataFinal": $('#txtDataFinal').val(), "HoraFim": $('#txtHoraFinal').val(), "KmFinal": $('#txtKmFinal').val(), "VeiculoId": veiculos.value, "MotoristaId": motoristas.value, "Finalidade": finalidade.value, "SetorSolicitanteId": setor.value[0], "EventoId": evento.value[0] })
    }
    else {
        var objViagem = JSON.stringify({ "ViagemId": $('#txtId').val(), "NoFichaVistoria": $('#txtNoFichaVistoria').val(), "DataInicial": $('#txtDataInicial').val(), "HoraInicial": $('#txtHoraInicial').val(), "KmInicial": $('#txtKmInicial').val(), "DataFinal": $('#txtDataFinal').val(), "HoraFim": $('#txtHoraFinal').val(), "KmFinal": $('#txtKmFinal').val(), "VeiculoId": veiculos.value, "MotoristaId": motoristas.value, "Finalidade": finalidade.value, "SetorSolicitanteId": setor.value[0] })
    }
